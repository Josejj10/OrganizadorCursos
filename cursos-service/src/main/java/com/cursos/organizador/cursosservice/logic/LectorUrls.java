package com.cursos.organizador.cursosservice.logic;
...
// Clase con MAIN en el que se puede probar el algoritmo
// No es llamado por el controller, El controller llama a LectorUrlsService

public class LectorUrls {
    ...
    private static void leerUrls() {
        ...
        UrlsPUCP.URLs.forEach((nom, link) -> {
                    ...
                    Carrera c = new Carrera(){{setNombre(nom);setLink(link);}};
                    leerUrl(link,c);
                }
        );
    ...
    private static void leerUrl(String u, Carrera carrera) {
        try{
            ...
            String obligatorios = new PDFTextStripper(){{
                setEndPage(1);
            }}.getText(doc);
            String electivos = new PDFTextStripper(){{
                setStartPage(2);
                setEndPage(2);
            }}.getText(doc);
            doc.close();
...
//        System.out.println("No hallado "+ codigo +", creando uno nuevo");
        Curso n = new Curso(){{setCode(codigo);}};
        cursos.add(n);
        return n;
    }
...
//        System.out.println("No hallado "+ codigo +", creando uno nuevo");
        return new Curso(){{setCode(codigo);}};
    }
